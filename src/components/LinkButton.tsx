...
const LinkButton: React.FC<LinkButtonProps> = ({ href, icon: Icon, text, isPDF = false }) => {
  ...
  const handleMouseEnter = () => {
    // Preconnect warmup on hover for faster navigation
    if (!href.startsWith('/') && !document.querySelector(`link[href="${new URL(href).origin}"]`)) {
  ...
  return (
    <button
      ...
      className="w-full max-w-md mx-auto bg-white hover:bg-gray-50 border border-gray-200 rounded-xl p-3 h-16 flex items-center justify-between enhanced-button group shadow-sm hover:shadow-lg link-warmup"
